...
percentage_human_human_file = 0
percentage_human_dog_file = 0

for i in range(100):
    if face_detector(human_files_short[i]) == True:
        percentage_human_human_file += 1
    if face_detector(dog_files_short[i]) == True:
        percentage_human_dog_file += 1

print('percent: {:.2%}'.format(percentage_human_human_file/100))
print('percent: {:.2%}'.format(percentage_human_dog_file/100))
...
# __回答:__
# 
# 我认为这是合理的，我们训练这个模型的特质就是基于清晰的面部特征，如果我们需要通过非清晰的面部特征来进行检索的话，比如说侧颜，我们就需要提供一个侧颜的数据集，来训练一个新的模型。
# 
# 
...
percentage_dog_dog_file = 0
percentage_dog_human_file = 0
for i in range(100):
    if dog_detector(human_files_short[i])==True:
        percentage_dog_human_file += 1
    if dog_detector(dog_files_short[i])==True:
        percentage_dog_dog_file += 1

print('percent: {:.2%}'.format(percentage_dog_human_file/100))
print('percent: {:.2%}'.format(percentage_dog_dog_file/100))
...
# 我选择了上图的网络搭建，理由如下：
# 
# 1.随着训练的深入，filter的不断变大，有助于模型更好的训练，因为一开始，模型可以进行部分的特征的识别，随着训练的深入，这些特征会形成一幅完整的图画；
# 2.池化层的作用是减少参数的数量，从而完成在网络上的计算；
# 3.GAP层可以用来降低三维张量的空间维度，展平为 1D，之后通过密度层；
# 4.ReLU激活再次用于除输出密度层之外的所有层，输出密集层使用softmax进行分类。
...
# __回答:__ 
# 
# 引入全局平均池化层，将2048个点直接softmax成133类，同时设置dropout为0.5，之所以选用Xception,是因为模型利用多个小卷积核代替一个大卷积核，有效的降维并且大幅度减少了参数
...
Xception_model.compile(loss='categorical_crossentropy', optimizer='adam', 
                  metrics=['accuracy'])
...
checkpointer = ModelCheckpoint(filepath='saved_models/Xception.weights.best.hdf5', 
                               verbose=1, save_best_only=True)
...
Xception_model.fit(train_Xception, train_targets, 
          ...
          epochs=20, batch_size=20, callbacks=[checkpointer, EarlyStopping], verbose=1, shuffle=True)
...
Xception_model.load_weights('saved_models/Xception.weights.best.hdf5')
...
def dog_human_detector(img_path):
    if face_detector(img_path):
        print("hello, human")
        showImage(img_path)
        dogname = Xception_predict_breed(img_path)
...
# 
# 1.输出结果不如我预想的好，；
# 2.（1）我觉得可以提供更多的训练数据，来进行训练，这样可以获得更好的参数；
#   （2）可以进行更多轮次的训练；
#   （3）或者采用数据增强的方法，例如翻转图片，旋转图片，或者降低学习率
...
dog_human_detector("images/Brittany_02625.jpg")


# In[37]:


dog_human_detector("images/American_water_spaniel_00648.jpg")


# In[39]:


dog_human_detector("images/Curly-coated_retriever_03896.jpg")


# In[40]:


dog_human_detector("images/Labrador_retriever_06449.jpg")


# In[43]:


dog_human_detector("images/3.jpg")


# In[42]:


dog_human_detector("images/2.jpg")
